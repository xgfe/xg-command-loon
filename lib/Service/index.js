...
function Service(option) {
  ...
  this.tmpdir = option.tmpdir || path.resolve(os.tmpdir(), `${uuid()}.loon.service`);
  this.config = option.config;
  this.changes = new Set();
  this.server = new Server({
    port: option.port,
    dirname: path.resolve(this.tmpdir, 'server'),
  });
  this.framework = new Framework({
    server: this.server,
    dirname: path.resolve(this.tmpdir, 'framework'),
...
Service.prototype.start = function() {
  return this.server.start().then(server => {
    ...
    const watcher = chokidar.watch('.', {
      cwd: this.dirname,
      ignored: fpath => /node_modules|\.git|\.swp|\.DS_Store/.test(fpath),
      persistent: true,
    });
    watcher.on('ready', () => {
