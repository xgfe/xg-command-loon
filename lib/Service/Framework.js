...
Framework.prototype.build = async function(framework) {
  const builder = {
    angular: async (framework) => {
      console.log(framework.target, this.server.dirname)
      ...
      angular.stdout.on('data', data => console.log(data));
      angular.stderr.on('data', data => console.log(data));
      angular.on('error', error => console.log(verbose));
      angular.on('close', code => console.log(verbose));
    },
