...
Gateway.prototype.handler = async function(request, response) {
  ...
  const proxy = this.framework.proxy || [];
  for (var i = 0; i < proxy.length; i++) {
    try {
      const config = proxy[i];
      if (typeof config === 'function') {
        return await config(request, response);
      } else {
...
Gateway.prototype.proxy = async function(request, response, config) {
  ...
  for (var i = 0; i < config.length; i++) {
    try {
      return await getProxyMiddleware(config[i])(request, response);
    } catch (e) {
    }
  }
